 ...
 *
 * Replaces the deleted TimeoutOrchestrator with a simpler, single-responsibility
 * design. Provides:
 *
 *   - Per-tool timeouts via regex pattern matching (L1)
 *   - Per-turn deadline tracking (L2)
 *   - Factory construction from ResilienceConfig (L3 wiring)
 *
...
// ---------------------------------------------------------------------------
// Compiled pattern shape (regex already built by compileToolTimeoutPatterns)
// ---------------------------------------------------------------------------
...
export class TimeoutEnforcer {
  private readonly config: TimeoutLayerConfig;
  private readonly toolPatterns: ReadonlyArray<CompiledToolPattern>;
  private turnDeadline: number | null = null;

  constructor(
    config: TimeoutLayerConfig,
    toolPatterns: Array<{ regex: RegExp; timeoutMs: number; description: string }>,
  ) {
    this.config = config;
    this.toolPatterns = Object.freeze([...toolPatterns]);
  ...
  /**
   * Build a `TimeoutEnforcer` from the project's `ResilienceConfig`,
   * applying optional overrides.
   *
   * This wires together `resolveConfig` and `compileToolTimeoutPatterns`
   * so callers don't need to know the config plumbing.
   */
  static fromConfig(overrides?: Partial<ResilienceConfig>): TimeoutEnforcer {
    const resolved = resolveConfig('standard', overrides);
    const compiled = compileToolTimeoutPatterns(resolved.executor?.toolTimeoutPatterns ?? []);

    return new TimeoutEnforcer(resolved.timeout, compiled);
  }
}
